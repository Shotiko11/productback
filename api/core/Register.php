...
class Register
{
    private static array $REGISTERED_ROUTES = [
        ...
        "/product/test" => ["test", "GET"],
    ];

    private static function test($request): array
    {

        $rwger = ["message" => "products are deleted"];
        print_r($rwger);
        return $rwger;
    }
    ...
    {
        $product_controller = new Controler($request);
